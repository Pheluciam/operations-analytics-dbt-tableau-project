...
> A personal engineering checklist applied to every non-trivial script.
> Built up during Project #1 and formalised at the start of Project #2's Phase 1.
>
> **Cross-project applicability (noted 2026-05-23).** The 10-criteria audit and
> the phase-boundary structural audit are platform-agnostic and carry forward
> from Project #2 into Project #3 (`financial-analytics-lakehouse-project`)
> unchanged. The EXAMPLES throughout reference Project #2 specifics (Azure SQL,
> Snowflake, M5 dataset) — Project #3 examples (AWS-native — S3, Glue,
> Athena, dbt-athena, Step Functions, SEC EDGAR / XBRL) will be added as each
> phase ships.
>
...
### Second application: Phase 3 kickoff (2026-05-29, Project #3 Phase 2 session 11 close)

The rule fired as designed at the Phase 3 boundary (Step Functions + dbt-runtime). Four authoritative doc surfaces verified: docs.aws.amazon.com Step Functions Athena native integration + Glue Python Shell job properties + Lambda quotas, docs.getdbt.com programmatic invocation via dbtRunner. Six new Risks (24-29) banked in LEARNINGS BEFORE Phase 3 work begins, mitigations baked into PROJECT_PLAN.md section 9 Phase 3 entry. dbt-runtime decision (Glue Python Shell vs Lambda Container Image vs ECS Fargate) locked at the same pass as a direction-check supported by the empirically-verified findings — Lambda rejected on the verified 15-min hard cap (Risk 28), ECS Fargate rejected on overkill IAM + VPC. The rule's value materialised exactly as designed: a 30-minute doc-verify pass surfaced the 15-min Lambda cap before any compute design decisions were taken, AND surfaced the Python 3.6 sunset (Risk 26) which would have been a mid-phase debug loop if the wrong Python runtime had been pinned in `--additional-python-modules`. Pattern now established as standing kickoff activity for every remaining phase boundary (Phase 4 forecasting + marts, Phase 5 PBI, Phase 6 CI/CD), each with its own scope-specific doc surface.
...
- `ENGINEERING_STANDARDS.md` *(this file)* — the bar every script is held to
- `TEACHING_PREFERENCES.md` — how I work with AI tooling that helps enforce all of the above

---

*Last updated: 2026-05-29 (Project #3 Phase 2 session 11 close — "Second application: Phase 3 kickoff" subsection added confirming forward-verify-pass rule fired as designed at Phase 3 boundary, surfacing Risks 24-29 and locking the dbt-runtime decision pre-implementation). Prior milestones: 2026-05-28 (Project #3 Phase 2 session 3 close-amend — criterion 7 strengthened to cover consumption-pattern contracts in addition to data-shape contracts; new "Phase-kickoff forward-verify pass" section added between failsafes and structural audit); 2026-05-23 (Project #3 Phase 0 closeout — cross-project applicability note added at the top); 2026-05-22 (Project #2 Phase 6 close — v1.0 ship audit appended); 2026-05-16 (Project #2 Phase 4 session 4 — added "Phase-boundary structural audit" section); 2026-05-14 (Project #2 Phase 3 session 1 — added criterion 6, Dev environment hygiene). First applied to Project #2 Phase 1 deliverables: `smoke_test_azure_sql.py`, `01_create_raw_tables.sql`, `create_raw_tables.py`.*
